...
public final class MDSReactionButton: UIControl {
    ...
    private func updateAppearance() {
        let sizeToken = SizeToken(size: size)
        let colorToken = ColorToken(variant: variant, isSelected: isSelected, isEnabled: isEnabled)
        ...
        guard size != .xsmall else { return }

        layer.cornerRadius = sizeToken.height / 2
        layer.masksToBounds = true
        layer.borderWidth = 1
        layer.borderColor = colorToken.stroke.resolvedColor(with: traitCollection).cgColor
...
private extension MDSReactionButton {
    ...
    struct ColorToken {
        ...
        init(variant: MDSReactionButton.Variant, isSelected: Bool, isEnabled: Bool) {
